...
export default function RegisterForm({ onSuccess }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'student', companyName: '', universityName: '', referralCode: params.get('ref') || '' });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => setForm({ ...form, role: 'university' })}
          className={`flex-1 py-2 rounded-lg font-medium border-2 transition-colors ${form.role === 'university' ? 'border-navy-800 bg-navy-50 text-navy-800' : 'border-gray-200 text-gray-500'}`}>
          University
        </button>
      </div>
      ...
      {form.role === 'university' && (
        <Input label="University Name" value={form.universityName} onChange={set('universityName')} required />
      )}
      ...
      {form.referralCode && <p className="text-xs text-emerald-600">Referred by code {form.referralCode}</p>}
